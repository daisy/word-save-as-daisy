...
<xsl:stylesheet
		...
		xmlns:Library="org.daisy.pipeline.word_to_dtbook.impl.Pipeline1Library"
    ...
    <xsl:function name="pf:default-locale" as="xs:string">
        <xsl:value-of select="Library:getDefaultLocale($_lib_)">
            <!--
                Implemented in java/org/daisy/pipeline/word_to_dtbook/impl/Pipeline1LibraryDefinition.java
            -->
        </xsl:value-of>
    </xsl:function>

    <!--Get the current date as string value in format 'YYYY-MM-dd'
    Note : this could be replace by fn:current-date() but the timezone might need to be stripped-->
    <!--<xsl:function name="pf:date-now" as="xs:string">
        <xsl:value-of select="Library:getDate($_lib_)">
